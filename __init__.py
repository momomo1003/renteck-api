...
@app.route("/number")
def get_number():
    sql = """SELECT pc_information.model,pc_information.manufacturer,Inventory_control FROM Inventory_control JOIN pc_information ON Inventory_control.pc_number=pc_information.pc_number WHERE Inventory_control.Inventory_control<>0 """
    db.execute_sql_query(sql)
    entries = populate_entries()
